...
expr({op,Line,Op,A0}) ->
    ...
    {op,Line,Op,A1};
expr({op, Line, '!', L0, R0}) ->
    {op, Line, '!', replace_pid_name(L0), R0};
...
replace_pid_name({atom, _, _} = Name) ->
    Name;
replace_pid_name({_, Line, _} = Name) ->
    {call, Line,
        {remote,Line,{atom,Line,gproc},{atom,Line,lookup_local_name}}, [Name]
    }.
